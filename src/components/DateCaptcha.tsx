...
interface DateCaptchaProps {
  correctDate?: string; // Format: DD/MM/YYYY
...
export default function DateCaptcha({ correctDate, onVerify }: DateCaptchaProps) {
  ...
  const finalCorrectDate = correctDate || "04/02/2026";
  ...
  useEffect(() => {
    if (day.length === 2 && month.length === 2 && year.length === 4) {
      ...
      const normalizedCorrectDate = finalCorrectDate.replace(/[.\s]/g, '/');
  ...
  return (
    <div className="w-full font-sans bg-transparent h-full flex flex-col pt-6 px-5 relative">
      ...
      <h3 className="text-[16px] font-black uppercase tracking-tight text-white mb-8 relative z-10 leading-snug">
        Bugünkü tarixi aşağıdakı düymələr vasitəsilə daxil edin:
      </h3>
      ...
      <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-4 mb-10 relative z-10 shadow-2xl">
        ...
        {error && (
          <div className="absolute -bottom-8 left-0 right-0 text-center text-red-400 text-sm font-black uppercase tracking-wider transition-all">
            {error}
          </div>
        )}
      </div>
          ...
          if (key === 'clear') {
            return (
              <button
                key="cl"
                onClick={handleClear}
                className="h-14 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center justify-center text-red-400 hover:bg-red-500 hover:text-white transition-all shadow-xl"
              >
